...
class LFunctionDataset1D(Dataset):
    """
    Custom PyTorch Dataset for L-function data (1D version).
    Extracts only the first column of the real channel (original a_p).
    """
    ...
    def __getitem__(self, idx):
        # data[idx] shape is (SIZE, SIZE, 2)
        feature_2d = np.array(self.data[idx])
        
        # Extract first column of the real channel.
        # Channel 0 is Real.
        # Assuming rows=primes, cols=characters, we want the first column (first character).
        # shape: (SIZE, )
        feature_1d = feature_2d[:, 0, 0]
        
        # Add channel dimension for Conv1d: (1, SIZE)
        feature_tensor = torch.FloatTensor(feature_1d).unsqueeze(0)
...
real_dataset = LFunctionDataset1D(real_data_mmap, label_value=1)
fake_dataset = LFunctionDataset1D(fake_data_mmap, label_value=0)
...
class LFunctionCNN1D(nn.Module):
    def __init__(self):
        super(LFunctionCNN1D, self).__init__()
        # Input: (Batch, 1, 500)
        self.conv_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2), # Output: (64, 250)

            # Block 2
            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=3, padding=1),
            nn.BatchNorm1d(128), # Batch Normalization
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2), # Output: (128, 125)

            # Block 3
            nn.Conv1d(in_channels=128, out_channels=256, kernel_size=3, padding=1),
            nn.BatchNorm1d(256), # Batch Normalization
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2), # Output: (256, 62)

            # Block 4
            nn.Conv1d(in_channels=256, out_channels=512, kernel_size=3, padding=1),
            nn.BatchNorm1d(512), # Batch Normalization
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2), # Output: (512, 31)

            nn.Conv1d(in_channels=512, out_channels=512, kernel_size=3, padding=1),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)  # Output: (512, 15)
        )
...
model = LFunctionCNN1D().to(DEVICE)
print("\nModel Architecture:")
print(model)
