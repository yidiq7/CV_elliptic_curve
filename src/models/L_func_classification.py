...
def parse_args():
    parser = argparse.ArgumentParser(description="L-function CNN Training (2D)")
    
    # Experiment Setup
    parser.add_argument("--image_size", type=int, default=200, help="Size of the input images")
    parser.add_argument("--resume", action="store_true", help="Resume training from checkpoint")
    parser.add_argument("--seed", type=int, default=420, help="Random seed for data splitting")
...
if RESUME_TRAINING:
    try:
        checkpoint = torch.load(os.path.join(RESULTS_DIR, f'L_function_classifier_{IMAGE_SIZE}_checkpoint.pth'), map_location=DEVICE)
...
torch.save(checkpoint, os.path.join(RESULTS_DIR, f'L_function_classifier_{IMAGE_SIZE}_checkpoint.pth'))
